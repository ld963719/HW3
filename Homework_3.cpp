...
// Создание мьютекса и двух глобальных переменных: номера выполняемой задачи и флага для определения выполнения максимального количества задач.
int task_number = 0;
int flag = 0;
mutex mtx;
...
int main()
{
	...
	// Генерирование и запуск потоков.
	thread t1;
	thread t2;
	thread t3;
	thread t4;
	...
	while (flag == 0)
	{
		...
		thread t1(write, N * M * K, 1, library_list, books);
		if (flag == 1)
		{
			break;
		}
		thread t2(write, N * M * K, 2, library_list, books);
		if (flag == 1)
		{
			break;
		}
		thread t3(write, N * M * K, 3, library_list, books);
		if (flag == 1)
		{
			break;
		}
		thread t4(write, N * M * K, 4, library_list, books);
		if (flag == 1)
		{
			break;
		}
		thread t5(write, N * M * K, 5, library_list, books);
		if (flag == 1)
		{
			break;
		}
